...
/* Source kind of a VNAV_OBSERVATION; selects the interpretation of the bearing/range fields. The navigator's estimator never sees the kind beyond normalization. */
type VnavObsKind uint32

const (
	/* Visual landmark recognizer: bearing (relative or true per flags) and optional ground range. */
	VNAV_OBS_KIND_VISUAL VnavObsKind = 0

	/* VOR receiver: bearing_deg is the radial (bearing FROM the station, in the station's calibration frame); use decl_cdeg. */
	VNAV_OBS_KIND_VOR VnavObsKind = 1

	/* ADF/NDB: bearing_deg is relative to the aircraft nose. */
	VNAV_OBS_KIND_NDB VnavObsKind = 2
...
type VnavObsFlags uint32

const (
	/* bearing_deg (and bearing_sigma_deg) are valid. */
	VNAV_OBS_FLAG_BEARING VnavObsFlags = 1

	/* bearing_deg is body-frame relative to the aircraft nose, positive right; clear means true bearing aircraft to reference (visual) or kind-specific absolute (VOR radial). */
	VNAV_OBS_FLAG_BEARING_RELATIVE VnavObsFlags = 2

	/* range_m (and range_frac_sigma) are valid. */
	VNAV_OBS_FLAG_RANGE VnavObsFlags = 4

	/* decl_cdeg is valid (VOR station declination of record). */
	VNAV_OBS_FLAG_DECL VnavObsFlags = 8
)

/* Status of a VNAV_POSE answer. */
type VnavPoseResult uint32

const (
	/* Pose fields are valid. */
	VNAV_POSE_OK VnavPoseResult = 0

	/* The navigator's estimator has not initialized yet; retry later. */
	VNAV_POSE_UNANCHORED VnavPoseResult = 1

	/* No attitude telemetry seen yet; retry later. */
	VNAV_POSE_NO_ATTITUDE VnavPoseResult = 2
)
...
func Dialect(mid int) mavlink.Message {
	...
	case 52101:
		return &HdgaltState{}
	case 52110:
		return &VnavObservation{}
	case 52111:
		return &VnavPoseRequest{}
	case 52112:
		return &VnavPose{}
	}
	return nil
...
/* A ranked-candidate reference-point observation for the navigator (visual landmark recognizer, VOR, NDB or DME). Candidates are the genuinely confusable set, pre-thinned by the observer; scores are relative weights, monotonic in confidence (calibration not required, honesty about uncertainty is), renormalized with null_score to sum 1 by the consumer. The null row is the first-class "this sighting corroborates nothing" outcome and is actively consumed; do not suppress it. */
type VnavObservation struct {
	/* Capture time of the underlying image/signal (same base as SYSTEM_TIME.time_boot_ms). Processing lag up to the navigator's window (default 6 min) is compensated; 0 means unknown and is treated as arrival time. */
	TimeBootMs uint32

	/* Bearing per kind and flags: relative to the nose (positive right) or true aircraft-to-reference; for VOR the radial. */
	BearingDeg float32

	/* 1-sigma bearing error; 0 = consumer default. */
	BearingSigmaDeg float32

	/* Range: horizontal ground range (visual), slant range (DME); see kind. */
	RangeM float32

	/* Fractional 1-sigma range error; 0 = consumer default. */
	RangeFracSigma float32

	/* Candidate latitudes (WGS84), degrees * 1e7. */
	CandLat [5]int32

	/* Candidate longitudes (WGS84), degrees * 1e7. */
	CandLon [5]int32

	/* VOR station declination of record, centidegrees east-positive (the magnetic alignment the station was calibrated to, not the current field). */
	DeclCdeg int16

	/* Candidate elevations AMSL (from the observer's database); 0 if unknown. */
	CandElevM [5]int16

	/* Observation source kind. */
	Kind VnavObsKind `mavlink:"byte"`

	/* Field-validity bits. */
	Flags VnavObsFlags `mavlink:"byte,bitmask"`

	/* Null-row weight, 0..255 scale. */
	NullScore byte

	/* Number of valid entries in the candidate arrays, 1..5. */
	NCandidates byte
...
func (m *VnavObservation) CRCExtra() byte { return 55 }

func (m *VnavObservation) MarshalV1(buf []byte) []byte {
	buf = marshalUint32(buf, (m.TimeBootMs))
	buf = marshalFloat32(buf, (m.BearingDeg))
	buf = marshalFloat32(buf, (m.BearingSigmaDeg))
	buf = marshalFloat32(buf, (m.RangeM))
	buf = marshalFloat32(buf, (m.RangeFracSigma))
	for _, v := range m.CandLat {
		buf = marshalInt32(buf, (v))
	}
	for _, v := range m.CandLon {
		buf = marshalInt32(buf, (v))
	}
	buf = marshalInt16(buf, (m.DeclCdeg))
	for _, v := range m.CandElevM {
		buf = marshalInt16(buf, (v))
	}
	buf = marshalByte(buf, byte(m.Kind))
	buf = marshalByte(buf, byte(m.Flags))
	buf = marshalByte(buf, (m.NullScore))
	buf = marshalByte(buf, (m.NCandidates))
	for _, v := range m.CandScore {
		buf = marshalByte(buf, (v))
	}
...
func (m *VnavObservation) UnmarshalV1(buf []byte) []byte {

	buf, m.TimeBootMs = unmarshalUint32(buf)

	buf, m.BearingDeg = unmarshalFloat32(buf)

	buf, m.BearingSigmaDeg = unmarshalFloat32(buf)

	buf, m.RangeM = unmarshalFloat32(buf)

	buf, m.RangeFracSigma = unmarshalFloat32(buf)

	for i, _ := range m.CandLat {
		buf, m.CandLat[i] = unmarshalInt32(buf)
	}

	for i, _ := range m.CandLon {
		buf, m.CandLon[i] = unmarshalInt32(buf)
	}

	buf, m.DeclCdeg = unmarshalInt16(buf)

	for i, _ := range m.CandElevM {
		buf, m.CandElevM[i] = unmarshalInt16(buf)
	}

	{
		var v byte
		buf, v = unmarshalByte(buf)
		m.Kind = VnavObsKind(v)
	}

	{
		var v byte
		buf, v = unmarshalByte(buf)
		m.Flags = VnavObsFlags(v)
	}

	buf, m.NullScore = unmarshalByte(buf)

	buf, m.NCandidates = unmarshalByte(buf)

	for i, _ := range m.CandScore {
		buf, m.CandScore[i] = unmarshalByte(buf)
	}
...
/* Ask the navigator for its believed aircraft pose at a capture time, to seed a landmark search (an ever-growing circle around the answer). The navigator replies with VNAV_POSE on the same link. */
type VnavPoseRequest struct {
	/* Capture time being asked about (same base as SYSTEM_TIME.time_boot_ms). */
	TimeBootMs uint32
}

func (m *VnavPoseRequest) ID() int        { return 52111 }
func (m *VnavPoseRequest) CRCExtra() byte { return 247 }

func (m *VnavPoseRequest) MarshalV1(buf []byte) []byte {
	buf = marshalUint32(buf, (m.TimeBootMs))

	return buf
}

func (m *VnavPoseRequest) MarshalV2(buf []byte) []byte {
	buf = m.MarshalV1(buf)

	return buf
}

func (m *VnavPoseRequest) UnmarshalV1(buf []byte) []byte {
...
func (m *VnavPoseRequest) UnmarshalV2(buf []byte) []byte {
	buf = m.UnmarshalV1(buf)

	return buf
}

/* The navigator's believed aircraft pose at the requested capture time, retrodicted consistently with how a delayed observation at that time will be fused. On a non-OK result only time_boot_ms and result are meaningful; back off and retry. */
type VnavPose struct {
	/* Echo of the request's capture time. */
	TimeBootMs uint32

	/* Believed latitude at capture time (WGS84), degrees * 1e7. */
	Lat int32

	/* Believed longitude at capture time (WGS84), degrees * 1e7. */
	Lon int32

	/* 1-sigma-ish horizontal uncertainty: a sensible initial search radius, not a bound. */
	PosSigmaM float32

	/* Barometric altitude AMSL, bias-corrected; soft (can carry a QNH-class offset). */
	AltM float32

	/* Roll at capture time, centidegrees, right positive. */
	RollCdeg int16

	/* Pitch at capture time, centidegrees, up positive. */
	PitchCdeg int16

	/* TRUE heading at capture time, centidegrees, 0..35999. */
	YawCdeg uint16

	/* Validity of the pose fields. */
	Result VnavPoseResult `mavlink:"byte"`

	/* Live hypothesis count; above 1 the navigator entertains multiple position stories and the search should widen. */
	Nhyp byte
...
func (m *VnavPose) CRCExtra() byte { return 22 }

func (m *VnavPose) MarshalV1(buf []byte) []byte {
	buf = marshalUint32(buf, (m.TimeBootMs))
	buf = marshalInt32(buf, (m.Lat))
	buf = marshalInt32(buf, (m.Lon))
	buf = marshalFloat32(buf, (m.PosSigmaM))
	buf = marshalFloat32(buf, (m.AltM))
	buf = marshalInt16(buf, (m.RollCdeg))
	buf = marshalInt16(buf, (m.PitchCdeg))
	buf = marshalUint16(buf, (m.YawCdeg))
	buf = marshalByte(buf, byte(m.Result))
	buf = marshalByte(buf, (m.Nhyp))
	buf = marshalByte(buf, (m.NullEma))
...
func (m *VnavPose) UnmarshalV1(buf []byte) []byte {
	...
	buf, m.AltM = unmarshalFloat32(buf)

	buf, m.RollCdeg = unmarshalInt16(buf)

	buf, m.PitchCdeg = unmarshalInt16(buf)

	buf, m.YawCdeg = unmarshalUint16(buf)

	{
		var v byte
		buf, v = unmarshalByte(buf)
		m.Result = VnavPoseResult(v)
	}

	buf, m.Nhyp = unmarshalByte(buf)

	buf, m.NullEma = unmarshalByte(buf)
