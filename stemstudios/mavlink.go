// Generated enums and structures for Mavlink dialect stemstudios #0 version 3
package stemstudios
...
/*          How to configure LEDs. We can:         - Set all LEDs to the first color in our colors array.         - Set up to 8 consecutive LEDs, starting from a given index, to colors provided in an array.         - Set the LED colors to change according to the flight mode.         - Turn all LEDs off (clear).        */
type LedConfigMode uint32

const (
	/* Set all LEDs in the target strip to the first color in our colors array. */
	LED_CONFIG_MODE_ALL LedConfigMode = 0

	/* Set up to 8 consecutive LEDs, starting from the given index, to the colors provided in the colors array. */
	LED_CONFIG_MODE_INDEX LedConfigMode = 1

	/* Set all LEDs in target strip to change color according to the flight mode. */
	LED_CONFIG_MODE_FOLLOW_FLIGHT_MODE LedConfigMode = 2

	/* Set all LEDs in the target strip to black (turn off). */
	LED_CONFIG_MODE_CLEAR LedConfigMode = 3
)
...
type LedStripConfig struct {
	...
	Mode LedConfigMode `mavlink:"byte"`
...
func (m *LedStripConfig) ID() int        { return 52600 }
func (m *LedStripConfig) CRCExtra() byte { return 181 }

func (m *LedStripConfig) MarshalV1(buf []byte) []byte {
...
func (m *LedStripConfig) MarshalV2(buf []byte) []byte {
	buf = m.MarshalV1(buf)

	return buf
}

func (m *LedStripConfig) UnmarshalV1(buf []byte) []byte {

	for i, _ := range m.Colors {
		buf, m.Colors[i] = unmarshalUint32(buf)
	}
...
func (m *LedStripConfig) UnmarshalV2(buf []byte) []byte {
	buf = m.UnmarshalV1(buf)
...
type LedStripState struct {
	...
	FollowingFlightMode byte
}

func (m *LedStripState) ID() int        { return 52601 }
func (m *LedStripState) CRCExtra() byte { return 102 }

func (m *LedStripState) MarshalV1(buf []byte) []byte {
	for _, v := range m.Colors {
		buf = marshalUint32(buf, (v))
	}
	buf = marshalByte(buf, (m.Length))
	buf = marshalByte(buf, (m.Index))
	buf = marshalByte(buf, (m.Id))
	buf = marshalByte(buf, (m.FollowingFlightMode))

	return buf
}

func (m *LedStripState) MarshalV2(buf []byte) []byte {
	buf = m.MarshalV1(buf)

	return buf
}

func (m *LedStripState) UnmarshalV1(buf []byte) []byte {
	...
	buf, m.FollowingFlightMode = unmarshalByte(buf)

	return buf
}

func (m *LedStripState) UnmarshalV2(buf []byte) []byte {
	buf = m.UnmarshalV1(buf)
